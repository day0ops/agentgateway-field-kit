...
export class InfraManager {
  ...
  async loadRegion(infraProfile) {
    const inlineRegion = InfraSchema.getRegion(infraProfile);
    if (inlineRegion) {
      return inlineRegion;
    }

    const environment = await this.loadEnvironment(infraProfile);
    return environment?.spec?.aws?.region || null;
  }
  ...
  async provision(options = {}) {
    ...
    const region = await this.loadRegion(infraProfile);
